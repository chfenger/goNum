...
func LEs_JocobiIterate(A, b, x0 goNum.Matrix, tol float64, n int) ([]float64, bool) {
	...
	*/
	...
	//求解
	for i := 0; i < n; i++ {
		x1 = goNum.AddMatrix(goNum.DotPruduct(B, x0), g)
...
func BenchmarkLEs_JocobiIterate(b *testing.B) {
	A16 := goNum.NewMatrix(3, 3, []float64{10.0, -2.0, -1.0, -2.0, 10.0, -1.0, -1.0, -2.0, 5.0})
	b16 := goNum.NewMatrix(3, 1, []float64{3.0, 15.0, 10})
	x16 := goNum.ZeroMatrix(3, 1)

	for i := 0; i < b.N; i++ {
		LEs_JocobiIterate(A16, b16, x16, 1e-6, 1e6)
	}
}
