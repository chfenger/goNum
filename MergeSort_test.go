...
// i1 --- mid
// i2 --- last
func mergeSort_merge(sol *goNum.Matrix, i0, i1, i2 int) {
	temp := goNum.ZeroMatrix(1, (*sol).Columns)
	i := i0
	j := i1 + 1
	k := 0
	for i <= i1 && j <= i2 {
...
// i0 --- first
// i2 --- last
func mergeSort_sort(sol *goNum.Matrix, i0, i2 int) {
	if i0 < i2 {
		var i1 int = (i0 + i2) / 2
...
// MergeSort 归并排序法
func MergeSort(in goNum.Matrix) (goNum.Matrix, bool) {
	/*
	      归并排序法
	   输入   :
	       in      输入矩阵, 1xn
...
func BenchmarkMergeSort(b *testing.B) {
	x62 := goNum.NewMatrix(1, 5, []float64{5.0, 3.2, 1.8, 2.4, 0.1})
	for i := 0; i < b.N; i++ {
		goNum.MergeSort(x62)
	}
}
