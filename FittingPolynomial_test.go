...
func FittingPolynomial(xy goNum.Matrix, m int) (goNum.Matrix, float64, float64, bool) {
	...
	*/
	...
	//求解矛盾方程组
	sol, err := goNum.InconsistentLSQ(A, goNum.Slices1ToMatrix(xy.ColumnOfMatrix(1)))
	//判断结果
	if err != true {
	...
	RMS = math.Sqrt(RMS)
	MaxErr, _, _ := goNum.MaxAbs(errSub)

	return sol, RMS, math.Abs(MaxErr), err
}

func BenchmarkFittingPolynomial(b *testing.B) {
	A33 := goNum.NewMatrix(7, 2, []float64{1.0, 10.0,
		3.0, 5.0,
		4.0, 4.0,
		5.0, 2.0,
		6.0, 1.0,
		...
		8.0, 2.0})
	for i := 0; i < b.N; i++ {
		goNum.FittingPolynomial(A33, 2) //~[13.4451,-3.5850,0.2639]
	}
}
