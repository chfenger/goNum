...
func BenchmarkLEs_SORIterate(b *testing.B) {
	A18 := goNum.NewMatrix(4, 4, []float64{2.0, -1.0, 0.0, 0.0,
		-1.0, 2.0, -1.0, 0.0,
		0.0, -1.0, 2.0, -1.0,
		0.0, 0.0, -1.0, 2.0})
	b18 := goNum.NewMatrix(4, 1, []float64{1.0, 0.0, 1.0, 0.0})
	x18 := goNum.NewMatrix(4, 1, []float64{1.0, 1.0, 1.0, 1.0})

	for i := 0; i < b.N; i++ {
		LEs_SORIterate(A18, b18, x18, 1e-6, 1.46, 1e6)
	}
}
