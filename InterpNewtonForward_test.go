...
func difff_InterpNewtonForward(A goNum.Matrix, k int) float64 {
	...
	for s := 1; s <= k; s++ {
		sol += math.Pow(-1.0, float64(s)) * float64(goNum.Cnm(k, s)) * A.GetFromMatrix(k-s, 1)
	}
	return sol
}

// InterpNewtonForward 计算x点n次Newton前向插值结果，拟合n+1个等距数据点
func InterpNewtonForward(A goNum.Matrix, xq float64) (float64, bool) {
	/*
		计算x点n次Newton前向插值结果，拟合n+1个等距数据点
		输入   :
		    A       数据点矩阵，(n+1)x2，第一列xi等距分布；第二列yi
	...
	*/
	...
	for k := 1; k < n+1; k++ {
		...
		//乘系数1/(k!h^k)
		BA.Data[k] = BA.Data[k] / (float64(goNum.Factorial(k)) * math.Pow(h, float64(k)))
		//求乘积
		for j := 0; j < k; j++ {
...
func BenchmarkInterpNewtonForward(b *testing.B) {
	A25 := goNum.NewMatrix(7, 2, []float64{
		0.50, 0.521095,
		0.55, 0.578152,
		0.60, 0.636654,
		0.65, 0.696748,
