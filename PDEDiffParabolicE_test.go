...
func PDEDiffParabolicE(funp, funu1, funu2 func(float64) float64, x0 goNum.Matrix,
	A, B float64, m, n int) (goNum.Matrix, bool) {
	/*
		求解抛物型偏微分方程的差分解法（显式）
		输入   :
		    funp, funu1, funu2   边界函数
...
func fun41_p(x float64) float64 {
	return 4.0 * x * (1.0 - x)
}

func fun41_u1(t float64) float64 {
	return 0.0
}

func fun41_u2(t float64) float64 {
	return 0.0
}

func BenchmarkPDEDiffParabolicE(b *testing.B) {
	x41 := goNum.NewMatrix(2, 2, []float64{0.0, 0.0, 1.0, 0.06})
	for i := 0; i < b.N; i++ {
		goNum.PDEDiffParabolicE(fun41_p, fun41_u1, fun41_u2, x41, 1.0, 0.0, 10, 36)
	}
}
