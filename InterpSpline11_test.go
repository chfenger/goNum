...
func InterpSpline11(A goNum.Matrix) (goNum.Matrix, bool) {
	...
	*/
	var err bool = false
	n := A.Rows - 1
	sol := goNum.ZeroMatrix(4, n)
	BA := goNum.ZeroMatrix(n-1, n-1) //对角占优的三对角矩阵
	BB := goNum.ZeroMatrix(n-1, 1)   //解向量
	BC := goNum.ZeroMatrix(n-1, 1)   //值向量
	...
	//1.1求解
	soltemp, errtemp := goNum.LEs_Chasing(BA, BC)
	if errtemp != true {
	...
	//2求解Si(x)
	S0 := goNum.ZeroMatrix(4, 1)
	S1 := goNum.ZeroMatrix(4, 1)
	S2 := goNum.ZeroMatrix(4, 1)
	S3 := goNum.ZeroMatrix(4, 1)
	for i := 1; i < n+1; i++ {
		...
		hi := xi - xi_1
		temp0 := goNum.ZeroMatrix(4, 1)
		temp1 := goNum.ZeroMatrix(4, 1)
		//2.1 S0
		temp0.Data[2] = 1.0
		temp0.Data[1] = -2.0 * xi
		temp0.Data[0] = xi * xi
...
func BenchmarkInterpSpline11(b *testing.B) {
	A30 := goNum.NewMatrix(4, 3, []float64{0.0, 0.0, 1.0,
		1.0, 3.0, 0.0,
		2.0, 4.0, 0.0,
		3.0, 6.0, 0.0})
	for i := 0; i < b.N; i++ {
		goNum.InterpSpline11(A30)
		// [0    -14/3 30  ]
		// [1    15    -37 ]
		// [14/3 -28/3 50/3]
		// [-8/3 2     -7/3]
